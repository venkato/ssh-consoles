...
import net.sf.jremoterun.utilities.nonjdk.swing.NameAndTextField2
...
//    private static final Logger log = LogManager.getLogger();
    ...
    void addPortForwardDialog(JptoTerminalPanel component, JptoSshJediTermWidget jedi) {
        JptoJSchShellTtyConnector ttyConnector = jedi.jSchShellTtyConnector;
        JDialog dialog = new JDialog(JrrSwingUtilsParent.findParentWindow(jedi, Window), "Add port fwd");
        Session session1 = ttyConnector.getSession()
        String[] localFwd = session1.getPortForwardingL()
        String[] remoteFwd = session1.getPortForwardingR()
        List<String> existed = []
        if(localFwd!=null&& localFwd.length>0){
            existed.addAll localFwd.toList().collect {'Listen local : '+it}
        }
        if(remoteFwd!=null&& remoteFwd.length>0){
            existed.addAll remoteFwd.toList().collect {'Listen remote : '+it}
        }

        JPanel panel =new JPanel4FlowLayout()
        NameAndTextField2 port1 = new NameAndTextField2('remote port', '', 4)
        NameAndTextField2 port2 = new NameAndTextField2('local port', '', 4)
        NameAndTextField2 host = new NameAndTextField2('host', '127.0.0.1', 10)
        JCheckBox checkBox=new JCheckBox('Listen remote',true)
        JCheckBox allInterfaces=new JCheckBox('Listen all interfaces',false)
        ...
        panel.add(button1)
        panel.add(checkBox)
        panel.add(allInterfaces)

        if(existed.size()>0){
            JTextArea area=new JTextArea(existed.join('\n'))
            area.editable = false
            panel.add(area)
        }
        button1.addActionListener {
            try {
                addPortForwardActual(component, jedi, port1.getText() as int, port2.getText() as int, host.getText(), checkBox.isSelected(),allInterfaces.isSelected())
                dialog.dispose()
            }catch (Throwable e){
                String additionMsg =''
                if( session1 instanceof JrrJschSession){
                    JrrJschSession jschSession = session1
                    additionMsg = jschSession.lastReceivedBytes2.toString()
                    byte[] buf3 = jschSession.lastReceivedBytes2.getBuf3()
                    //byte cmd=-1
                    if(buf3.length>6){
                        byte cmd= buf3[5]
                        additionMsg=cmd+' '+additionMsg
                    }
                }
                log.info("Failed add port ${additionMsg}",e)
                JrrUtilitiesShowE.showException("Failed add port ${additionMsg}",e)
            }
        }
    ...
    public static String allHostIp = '*'

    void addPortForwardActual(JptoTerminalPanel component, JptoSshJediTermWidget jedi, int localPort, int remotePort, String host, boolean listenRemote,boolean allHostsLisner) {
        JptoJSchShellTtyConnector ttyConnector = jedi.jSchShellTtyConnector;
        if(listenRemote) {
            if(allHostsLisner){
                ttyConnector.getSession().setPortForwardingR(allHostIp,localPort, host, remotePort)
            }else {
                ttyConnector.getSession().setPortForwardingR(localPort, host, remotePort)
            }
        }else {
            if(allHostsLisner){
                ttyConnector.getSession().setPortForwardingL(allHostIp,remotePort, host, localPort)
            }else {
                ttyConnector.getSession().setPortForwardingL(remotePort, host, localPort)
            }
        }
    }
    ...
    void getActions1(ArrayList<TerminalAction> actions) {
        ...
        actions.add(new JptoTerminalAction(SshKeyStokesShortCuts.portForward, { Component component ->
            log.info "port fwd called ${component.class.name}"
            if (component instanceof JptoTerminalPanel) {
                Container parent = component.getParent().getParent()
                JptoTerminalPanel term = (JptoTerminalPanel) component;
                if (parent instanceof JptoSshJediTermWidget) {
                    JptoSshJediTermWidget sshWi = (JptoSshJediTermWidget) parent;
                    addPortForwardDialog(term, sshWi)
                    return true;
                }
            }
            return false;
